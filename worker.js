...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}button,input,select,textarea{font:inherit}button{cursor:pointer}.wrap{max-width:1240px;margin:0 auto;padding:0 18px 28px}.hero{background:#fff;border-bottom:1px solid var(--border);margin:0 -18px 20px;padding:18px 18px 0}.hero-img{min-height:128px;padding:22px 12px 18px;display:flex;flex-direction:column;justify-content:center}.badge{display:inline-flex;width:max-content;background:#ecfdf5;color:#06a044;border-radius:999px;padding:6px 12px;font-weight:950;border:1px solid #bbf7d0}.hero h1{font-size:30px;margin:12px 0 6px;font-weight:950}.hero p{max-width:760px;line-height:1.65;margin:0;color:#64748b;font-weight:800}.tabs{display:flex;gap:8px;overflow:auto;padding:12px 0 14px;background:#fff}.tab{border:1px solid var(--border);background:#fff;border-radius:999px;padding:9px 14px;font-weight:950;white-space:nowrap;color:#475569}.tab.active{background:#06c755;border-color:#06c755;color:#fff}.grid{display:grid;grid-template-columns:minmax(0,1fr) 390px;gap:20px;align-items:start}.product-panel,.cart{background:#fff;border:1px solid var(--border);border-radius:8px;overflow:hidden;box-shadow:0 1px 2px rgba(15,23,42,.04)}.section-head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:18px 20px;border-bottom:1px solid var(--border)}.section-title{font-size:22px;font-weight:950}.section-note{color:#64748b;font-weight:800;font-size:13px}.products{display:grid}.product-row{display:grid;grid-template-columns:92px minmax(0,1fr) 96px 82px 92px 86px;gap:18px;align-items:center;padding:18px 20px;border-bottom:1px solid #eef2f7}.product-row:last-child{border-bottom:0}.product-img{width:72px;height:72px;border-radius:14px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;overflow:hidden;color:#94a3b8;font-weight:900}.product-img img{width:100%;height:100%;object-fit:cover}.store{font-size:12px;font-weight:950;color:#94a3b8}.name{font-size:17px;font-weight:950;margin:4px 0}.desc{color:#64748b;font-weight:800;line-height:1.45;font-size:13px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.pill{display:inline-flex;width:max-content;border-radius:8px;background:#ecfdf5;color:#059669;padding:5px 8px;font-size:12px;font-weight:950;margin-top:5px}.price{font-size:18px;font-weight:950}.points{color:#06c755;font-weight:950}.stock{font-size:13px;color:#475569;font-weight:900}.add{border:0;background:#eaf3ff;color:#2563eb;border-radius:8px;padding:10px 12px;font-weight:950}.add:hover{background:#dbeafe}.cart{position:sticky;top:14px}.cart h2{margin:0;padding:18px 20px;border-bottom:1px solid var(--border);font-size:20px;font-weight:950}.cart-inner{padding:16px;display:grid;gap:12px}.cart-row{display:grid;grid-template-columns:1fr auto;gap:8px;border-bottom:1px solid #f1f5f9;padding-bottom:10px}.qty{display:flex;align-items:center;gap:6px}.qty button{width:28px;height:28px;border-radius:8px;border:1px solid var(--border);background:#fff;font-weight:900}.total{display:flex;justify-content:space-between;font-size:20px;font-weight:950;border-top:1px solid var(--border);padding-top:12px}.field{display:grid;gap:6px}.field span{font-size:13px;color:#64748b;font-weight:900}.field input,.field select,.field textarea{width:100%;border:1px solid #cbd5e1;border-radius:12px;padding:11px 12px;background:#fff;font-weight:800}.field textarea{min-height:76px;resize:vertical}.hint{font-size:12px;color:#64748b;font-weight:800;line-height:1.5}.remit{background:#fff7ed;border:1px solid #fed7aa;border-radius:12px;padding:12px;color:#9a3412;font-weight:800;white-space:pre-wrap}.submit{border:0;background:#06c755;color:#fff;border-radius:14px;padding:15px;font-size:18px;font-weight:950}.submit:disabled{opacity:.55}.status{font-weight:900;line-height:1.55}.ok{color:#047857}.err{color:#dc2626}.empty{padding:36px;text-align:center;color:#64748b;font-weight:900;background:#fff}@media(max-width:980px){.grid{grid-template-columns:1fr}.cart{position:static}.wrap{padding:0 8px 22px}.hero{margin:0 -8px 14px;padding:14px 16px 0}.hero-img{min-height:96px;padding:12px 0}.hero h1{font-size:24px}.product-row{grid-template-columns:78px minmax(0,1fr);gap:13px;padding:14px}.product-row .price,.product-row .points,.product-row .stock,.product-row .add{grid-column:2}.product-row .price{margin-top:4px}.add{width:100%;}.section-head{padding:16px 14px}.section-title{font-size:20px}}  </style>
</head>
<body>
  <main class="wrap">
    <section class="hero">
    ...
    <section class="grid">
      <section class="product-panel"><div class="section-head"><div><div class="section-title">商城商品</div><div class="section-note">HookTea 商品清單，點選加入購物車後再結帳。</div></div><div class="section-note" id="productCount"></div></div><div class="products" id="productGrid"></div></section>
      <aside class="cart">
        <h2>購物車 / 結帳</h2>
  ...
  <script>
    ...
    function renderProducts(){ const rows=products.filter(p=>activeCategory==="ALL"||p.category===activeCategory); const count=$("productCount"); if(count)count.textContent=rows.length+" 項商品"; $("productGrid").innerHTML=rows.map(p=>'<article class="product-row"><div class="product-img">'+(p.image?'<img src="'+esc(p.image)+'" alt="">':'<span>□</span>')+'</div><div><div class="name">'+esc(p.name)+'</div><div class="store">'+esc(p.storeName||"HookTea")+'</div><div class="desc">'+esc(p.subtitle||p.description||"")+'</div><span class="pill">'+esc(p.badge||p.category||"商品")+'</span></div><div class="price">'+money(p.price)+'</div><div class="points">'+Number(p.pointsPrice||0).toLocaleString("zh-TW")+' 點</div><div class="stock">'+(p.stockUnlimited?'不限量':'庫存 '+Number(p.stockQty||0).toLocaleString("zh-TW"))+'</div><button class="add" data-add="'+esc(p.id)+'">加入</button></article>').join('')||'<div class="empty">目前沒有上架商品</div>'; document.querySelectorAll('[data-add]').forEach(btn=>btn.onclick=()=>addCart(btn.dataset.add)); }
