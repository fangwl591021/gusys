...
async function syncShopMemberProfile(request, env) {
  ...
  if (!row) {
    ...
    await env.DB.prepare(`
      INSERT INTO customers (id, company_id, line_user_id, display_name, picture_url, status, first_seen_at, created_at, updated_at)
      VALUES (?, 'default', ?, ?, ?, 'active', datetime('now'), datetime('now'), datetime('now'))
    `).bind(customerId, lineUserId, displayName, pictureUrl).run();
    row = { id: customerId };
  } else {
    await env.DB.prepare(`
      UPDATE customers
      SET display_name = CASE WHEN ? <> '' THEN ? ELSE display_name END,
          ...
          updated_at = datetime('now')
      WHERE id = ?
    `).bind(displayName, displayName, pictureUrl, pictureUrl, row.id).run();
  }
  const customer = await env.DB.prepare(`
    ...
    LIMIT 1
  `).bind(row.id).first();
  return json({ ok: true, data: customer || { id: row.id, lineUserId, displayName, pictureUrl } });
}
...
async function listAdminOrders(request, env) {
  ...
  const { results } = await env.DB.prepare(`
    SELECT o.id, o.order_no AS orderNo, o.status, o.payment_status AS paymentStatus,
           o.subtotal, o.discount, o.total, o.note, o.ordered_at AS orderedAt,
...
async function renderShopPage(env) {
  ...
  const data = JSON.stringify({ settings, products, loadError, brandTitle, heroTitle, heroSubtitle, heroImage }).replace(/</g, "\\u003c");
...
<body>
  <main class="app">
    ...
    <section class="hero" id="heroBanner"><div class="hero-copy"><div class="hero-title" id="heroTitle"></div><div class="hero-sub" id="heroSubtitle"></div></div></section>
  ...
  <aside class="checkout-sheet" id="checkoutSheet" aria-label="購物車結帳">
    ...
    <div class="sheet-body">
      <section class="member-panel" id="memberPanel"><div class="member-photo" id="memberPanelPhoto">U</div><div><div class="member-name" id="memberPanelName">尚未登入</div><div class="member-meta" id="memberPanelUid">請用 LINE LIFF 開啟以取得身分</div></div><div class="member-tags"><div class="member-chip">會員狀態<b id="memberPanelStatus">未登入</b></div><div class="member-chip">目前點數<b id="memberPanelPoints">未讀取</b></div><div class="member-chip">業務歸屬<b id="memberPanelSales">未綁定</b></div><div class="member-chip">加入日期<b id="memberPanelDate">-</b></div></div></section>
  ...
  <script>
    ...
    let settings=initial.settings||{}, products=initial.products||[], activeCategory="ALL", cart=[], memberProfile=null;
    ...
    function addCart(id){const p=products.find(x=>x.id===id);if(!p)return;const row=cart.find(x=>x.id===id);if(row)row.quantity+=1;else cart.push({id:p.id,name:p.name,price:Number(p.price||0),quantity:1});renderCart()}
    ...
    function handleNav(nav, btn){activateNav(btn); if(nav==="explore"){activeCategory="ALL";renderAll();window.scrollTo({top:0,behavior:"smooth"});showToast("已回到探索首頁");return} if(nav==="shop"){activeCategory="ALL";renderAll();scrollProducts();showToast("商城商品已更新");return} if(nav==="video"){showToast("影音內容尚未設定");return} if(nav==="orders"){openCart();showToast("訂單紀錄需依 LINE UID 串接");return} if(nav==="me"){openCart();showToast(memberProfile?"已載入個人專區":"請先完成 LINE 登入");return}}
    ...
    async function initLiff(){try{updateMemberPanel(null);if(!settings.liff_id||!window.liff){showToast("尚未設定 LIFF ID");return}await liff.init({liffId:settings.liff_id});if(!liff.isLoggedIn()){showToast("正在進行 LINE 登入");liff.login({redirectUri:location.href.split("#")[0]});return}const profile=await liff.getProfile();$("lineUserId").value=profile.userId||"";if(!$("shippingName").value)$("shippingName").value=profile.displayName||"";await syncMemberProfile(profile);}catch(e){console.warn(e);showToast("LINE 登入失敗："+(e.message||e))}}
    function updateMemberPanel(profile){const name=profile?.displayName||profile?.name||"尚未登入";const uid=profile?.lineUserId||profile?.userId||"請用 LINE LIFF 開啟以取得身分";const pic=profile?.pictureUrl||"";$("memberPanelName").textContent=name;$("memberPanelUid").textContent=uid;$("memberPanelStatus").textContent=profile?"已登入":"未登入";$("memberPanelSales").textContent=profile?.salesName||profile?.salesCode||"未綁定";$("memberPanelDate").textContent=(profile?.firstSeenAt||"").slice(0,10)||"-";$("memberPanelPhoto").innerHTML=pic?'<img src="'+esc(pic)+'" alt="">':(name||"U").slice(0,1);$("lineAvatar").innerHTML=pic?'<img src="'+esc(pic)+'" alt="">':(name||"U").slice(0,1);}
    async function syncMemberProfile(profile){memberProfile={lineUserId:profile.userId,displayName:profile.displayName,pictureUrl:profile.pictureUrl||""};updateMemberPanel(memberProfile);try{const res=await fetch('/api/shop/member',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(memberProfile)});const body=await res.json();if(res.ok&&body.ok&&body.data){memberProfile=Object.assign({},memberProfile,body.data);updateMemberPanel(memberProfile)}await loadMemberPoints(memberProfile.lineUserId);}catch(e){console.warn(e);showToast("會員資料同步失敗")}}
    async function loadMemberPoints(lineUserId){if(!lineUserId)return;$("memberPanelPoints").textContent="讀取中";try{const res=await fetch('/api/points/list?pointType=all&lineUserId='+encodeURIComponent(lineUserId));const data=await res.json();const nested=data?.data?.data?.data||data?.data?.data||data?.data||{};const logs=Array.isArray(data.logs)?data.logs:(Array.isArray(nested.list)?nested.list:(Array.isArray(data.items)?data.items:[]));const total=logs.length?logs.reduce((sum,log)=>sum+(Number(log.get_point||log.points||log.amount||log.point||0)||0),0):Number(data.balance||0)||0;$("memberPanelPoints").textContent=total.toLocaleString("zh-TW");}catch(e){$("memberPanelPoints").textContent="讀取失敗"}}
...
<body>
      ...
      <div class="shop-module-page">
        ...
        <div class="shop-layout hooktea-shop-layout">
          <div class="shop-module-forms">
            ...
            <section class="settings-card shop-tab-panel" id="shopTab-payment" style="display:none"><div class="section-title shop-section-payment">付款與訂單模組</div><div class="form-grid hooktea-form-grid"><label><span class="input-label">付款方式</span><input data-setting="shop_payment_methods" class="input-field" placeholder="LINEPAY,REMITTANCE,COD"></label><label><span class="input-label">LINE Pay 環境</span><select data-setting="linepay_env" class="input-field"><option value="sandbox">Sandbox 測試</option><option value="production">Production 正式</option></select></label><label class="full-span"><span class="input-label">匯款資訊</span><textarea data-setting="remittance_info" class="input-field"></textarea></label></div></section>
          </div>
  ...
  <script>
    ...
    let adminToken = localStorage.getItem("gusys_admin_token") || ""; let adminCustomers = []; let adminProducts = []; let adminOrders = []; let activeOrder = null; let broadcastData = {tags:[],campaigns:[],members:[],modules:[]}; let replyRules = []; let activeReplyRule = null; let activeCustomer = null; let activePointCustomer = null; const pointBalanceCache = {}; let richMenus = []; let activeRichMenu = null; let hookteaSettings = null; let smartMonitorData = {threads:[],messages:[],insights:[],selected:null}; let activeSmartThreadId = ""; let activeSmartRisk = "ALL"; const qs = s => document.querySelector(s); const qsa = s => Array.from(document.querySelectorAll(s));
    ...
    function adminOrderAmountText(order){ return "$"+money(order?.amount||0); }
    ...
    function setupSmartMonitorEvents(){ on("#smartSearch","input",renderSmartMonitor); on("#smartReload","click",loadAi); on("#smartAnalyze","click",analyzeSmartThread); on("#smartSearchBtn","click",renderSmartMonitor); on("#smartSend","click",()=>{ qs("#smartAiStatus").textContent="目前為人工送出預備區，尚未自動回覆"; }); on("#smartMarkPending","click",()=>{ qs("#smartAiStatus").textContent="已標示待處理"; }); on("#smartMarkDone","click",()=>{ qs("#smartAiStatus").textContent="已標示處理完畢"; }); qsa("[data-smart-risk]").forEach(btn=>btn.onclick=()=>{ activeSmartRisk=btn.dataset.smartRisk||"ALL"; qsa("[data-smart-risk]").forEach(b=>b.classList.toggle("active",b===btn)); renderSmartMonitor(); }); }
