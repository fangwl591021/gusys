...
品牌：重口味溫泉魚。名稱來自店家現場觀察：雙腳氣味較明顯時，魚群互動通常較集中，因此取名「重口味溫泉魚」。實際互動情況會因個人體質、飲食與作息等差異而不同；店家觀察多數男性較明顯、女性較少、兒童更少。品牌定位僅限休閒娛樂、溫泉魚互動、礁溪特色體驗。
收費：成人票 100 元；兒童票 80 元，適用 4 歲至國小六年級；4 歲以下免費。假日原價，平日優惠以現場公告為準。當天不限時間，蓋章可重複進場。目前僅收現金，不支援 LINE Pay。
體驗：現場共有 12 種不同互動感受，不同魚池的魚種、大小與接觸感不同。第一次或怕癢者可從沒有編號、較溫和的魚池開始；每個人的敏感度不同，一般約 3 分鐘後會逐漸適應，不保證特定感受。
...
入池與安全：需脫鞋襪。體驗前不用先洗腳，可直接脫鞋襪入池；但腳上若有沙子、泥土、乳液、藥膏或其他用品，必須先清洗乾淨。體驗結束離開前一定要洗腳。較大、開放性或持續滲血的傷口不得入池；一般已結痂、脫皮、蚊蟲咬傷紅點、輕微刮傷，或已用膠帶或敷料完整覆蓋的部位可以入池，如有疑慮請先詢問現場工作人員。糖尿病、洗腎或嚴重皮膚病者請勿入池。現場贈送毛巾一條，也可自備毛巾。池內不可站立、走動、跨池、抓魚、追魚、餵食或丟入物品；場內不可奔跑或推擠。兒童須由成人陪同。可拍照錄影，但須注意安全與他人隱私。
...
醫療與保健規範：重口味溫泉魚僅提供休閒娛樂及互動體驗，不宣稱具有醫療、治療、保健、改善健康、改善皮膚、改善循環或其他療效。消費者詢問相關效果時，只能回覆：「重口味溫泉魚提供的是休閒互動體驗，我們不提供醫療、治療或保健效果宣稱；體驗重點是好玩、有趣，許多來客會在互動過程中感到開心。」不得延伸回答任何效果、診斷或健康建議。
`.trim();
const LINE_AI_CHAT_SCOPE_PATTERN = /重口味|溫泉魚|礁溪|魚池|魚種|怕癢|體驗|特色|你們|店家|店裡|這間|門票|票價|收費|價格|費用|多少錢|付款|免費|幾歲|歲以下|國小|小六|兒童|小孩|親子|大人|長輩|營業|幾點|公休|今天有開|有開嗎|過年|颱風|下雨|地址|在哪|位置|導航|交通|怎麼去|火車站|停車|機車|遊覽車|入池|洗腳|鞋襪|毛巾|乳液|藥膏|傷口|糖尿病|洗腎|皮膚病|安全|滑|站立|走動|抓魚|摸魚|餵魚|拍照|影片|飲食|寵物|嬰兒車|輪椅|行李|預約|團體|包場|電話|客服|遺失|合作|順遊|景點|附近|美食|半日遊|集點|點數|現金|line\s*pay|優惠|活動|商城|媒體|分享好友|常見問題|faq|療效|治療|保健|改善健康|改善皮膚|改善循環|健康效果/i;
const LINE_AI_MEDICAL_CLAIM_PATTERN = /療效|治療|保健|改善健康|改善皮膚|改善循環|健康效果|對腳.*(?:效果|幫助)|皮膚.*變好|循環.*改善/i;
const LINE_AI_MEDICAL_CLAIM_REPLY = "🐟 重口味溫泉魚提供的是休閒互動體驗。\n\n我們不提供醫療、治療或保健效果宣稱。\n\n體驗重點是好玩、有趣，許多來客會在互動過程中感到開心。";
...
const LINE_JIAOXI_RECOMMENDATIONS = {
  ...
  visit: {
    items: [
      ...
      { name: "猴洞坑瀑布", phone: "039881311", phoneLabel: "管理洽詢", image: "https://travel.yilan.gov.tw/_api/content/images/attractions/10920/1920x1080_attractions-image-lf_vqebszuannmggk4a4la.jpg", imageSource: "宜蘭勁好玩", imageSourceUrl: "https://travel.yilan.gov.tw/zh-tw/attraction/96/" },
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname.startsWith("/api/admin/media-videos/") && request.method === "DELETE") return await deleteAdminMediaVideo(request, env, decodeURIComponent(url.pathname.slice("/api/admin/media-videos/".length)));
...
async function resolveLineOaIdentity(env, settings = {}) {
  ...
  if (configuredUrl) return { displayName, addFriendUrl: configuredUrl, chatUrl: configuredUrl };
  const token = String(env.LINE_CHANNEL_ACCESS_TOKEN || "").trim();
  if (!token) return { displayName, addFriendUrl: "", chatUrl: "" };
  try {
    const response = await fetch("https://api.line.me/v2/bot/info", {
      headers: { authorization: `Bearer ${token}` },
    });
    if (!response.ok) return { displayName, addFriendUrl: "", chatUrl: "" };
    ...
    return {
      ...
      chatUrl: basicId ? `https://line.me/R/oaMessage/${encodeURIComponent(basicId)}/` : "",
    };
  } catch (_) {
    return { displayName, addFriendUrl: "", chatUrl: "" };
  }
}
...
async function generateLineAiChatDecision(env, target, limits) {
  ...
  const prompt = [
    ...
    "若使用者詢問本體驗是否具有療效、治療、保健、改善健康、改善皮膚或改善循環等效果，只能依店家核准資料中的醫療與保健規範回覆，不得自行補充或推論。",
...
async function buildLineAiChatReplyDecision(env, events) {
  ...
  if (LINE_AI_MEDICAL_CLAIM_PATTERN.test(target.keyword)) {
    await updateLineAiReplyUsage(env, target.eventKey, "success", LINE_AI_MEDICAL_CLAIM_REPLY);
    return {
      handled: true,
      eventKey: target.eventKey,
      outcome: "medical_claim_guard",
      replyPayload: { replyToken: target.replyToken, messages: [{ type: "text", text: LINE_AI_MEDICAL_CLAIM_REPLY }] },
      summary: { handled: true, outcome: "medical_claim_guard", keyword: target.keyword, aiUsed: false },
    };
  }
...
async function ensureMediaVideoSchema(env) {
  await env.DB.batch([
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS media_videos (
      id TEXT PRIMARY KEY,
      title TEXT NOT NULL,
    ...
    env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_media_videos_status_order ON media_videos(status, display_order, created_at)"),
  ]);
}

function mediaVideoSelectSql(where = "") {
  return `SELECT id, title, description, media_label AS mediaLabel,
                 source_url AS sourceUrl, source_label AS sourceLabel,
                 playback_mode AS playbackMode, poster_url AS posterUrl,
                 display_order AS displayOrder, status,
                 created_at AS createdAt, updated_at AS updatedAt
          FROM media_videos ${where}
          ORDER BY display_order ASC, datetime(created_at) ASC`;
}

function normalizeHttpsUrl(value, field, required = false) {
  const raw = String(value || "").trim();
  if (!raw && !required) return "";
  let url;
  try { url = new URL(raw); } catch { throw new HttpError(400, `${field}_invalid`); }
  if (url.protocol !== "https:") throw new HttpError(400, `${field}_https_required`);
...
async function listAdminMediaVideos(request, env) {
  ...
  requireDb(env);
  await ensureMediaVideoSchema(env);
  const { results } = await env.DB.prepare(mediaVideoSelectSql()).all();
  return json({ ok: true, data: results || [] });
}

async function saveAdminMediaVideo(request, env) {
  requireAdmin(request, env);
  requireDb(env);
  await ensureMediaVideoSchema(env);
  const payload = await request.json().catch(() => ({}));
  const id = String(payload.id || crypto.randomUUID()).trim().slice(0, 100);
  const title = String(payload.title || "").trim().slice(0, 120);
  const description = String(payload.description || "").trim().slice(0, 1000);
  const mediaLabel = String(payload.mediaLabel || "官方影音").trim().slice(0, 60) || "官方影音";
  const sourceUrl = normalizeHttpsUrl(payload.sourceUrl, "media_source_url", true);
  const sourceLabel = String(payload.sourceLabel || "開啟店家影音").trim().slice(0, 60) || "開啟店家影音";
  const playbackMode = ["external", "html5"].includes(String(payload.playbackMode || "")) ? String(payload.playbackMode) : "external";
  const posterUrl = normalizeHttpsUrl(payload.posterUrl, "media_poster_url", false);
  const displayOrder = Math.min(9999, Math.max(-9999, Math.trunc(Number(payload.displayOrder || 0)) || 0));
  const status = String(payload.status || "active") === "inactive" ? "inactive" : "active";
  if (!title) throw new HttpError(400, "media_title_required");
  const before = await env.DB.prepare(mediaVideoSelectSql("WHERE id = ?")).bind(id).first();
  await env.DB.prepare(`
    INSERT INTO media_videos (
      id, title, description, media_label, source_url, source_label,
      playback_mode, poster_url, display_order, status, created_at, updated_at
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      updated_at = datetime('now')
  `).bind(id, title, description, mediaLabel, sourceUrl, sourceLabel, playbackMode, posterUrl, displayOrder, status).run();
  const saved = await env.DB.prepare(mediaVideoSelectSql("WHERE id = ?")).bind(id).first();
  await writeAudit(request, env, before ? "update_media_video" : "create_media_video", "media_video", id, before || {}, saved || {});
  return json({ ok: true, data: saved });
}

async function deleteAdminMediaVideo(request, env, id) {
  requireAdmin(request, env);
  requireDb(env);
  await ensureMediaVideoSchema(env);
  const videoId = String(id || "").trim();
  if (!videoId) throw new HttpError(400, "media_video_id_required");
  const existing = await env.DB.prepare(mediaVideoSelectSql("WHERE id = ?")).bind(videoId).first();
  if (!existing) throw new HttpError(404, "media_video_not_found");
  await env.DB.prepare("DELETE FROM media_videos WHERE id = ?").bind(videoId).run();
  await writeAudit(request, env, "delete_media_video", "media_video", videoId, existing, {});
  return json({ ok: true, data: { deleted: true, id: videoId } });
}
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--dark);font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;font-size:14px}button,input,select,textarea{font:inherit}
    ...
    .main-content{margin-left:var(--sidebar-width);min-height:100vh;transition:margin-left .18s ease}.page-header{position:sticky;top:0;z-index:10;background:rgba(255,255,255,.96);border-bottom:1px solid var(--border);padding:14px 22px;display:flex;align-items:center;justify-content:space-between;gap:16px}.page-title{font-size:21px;font-weight:800}.page-subtitle{margin-top:3px;color:var(--muted)}.header-actions{display:flex;align-items:center;gap:8px}.content{padding:20px 22px 36px;max-width:none}.view{display:none}.view.active{display:block}.admin-toast{position:fixed;top:88px;right:22px;z-index:220;min-width:260px;max-width:min(420px,calc(100vw - 32px));padding:14px 18px;border-radius:10px;background:#065f46;color:#fff;font-weight:900;box-shadow:0 18px 42px rgba(15,23,42,.24);opacity:0;transform:translateY(-12px);pointer-events:none;transition:.18s}.admin-toast.show{opacity:1;transform:translateY(0)}.admin-toast.error{background:#b91c1c}.admin-toast.info{background:#1d4ed8}.header-actions button:disabled{opacity:.62;cursor:wait}
    ...
    .media-admin-grid{display:grid;grid-template-columns:minmax(320px,430px) minmax(0,1fr);gap:14px;align-items:start}.media-form-actions,.media-row-actions{display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap}.media-url{max-width:360px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.media-description{min-height:110px}.media-help{margin-top:6px;color:var(--muted);font-size:12px;line-height:1.5}
    @media(max-width:980px){.sidebar,body.sidebar-collapsed .sidebar{position:static;width:auto}.main-content,body.sidebar-collapsed .main-content{margin-left:0}.page-header{position:static;align-items:flex-start;flex-direction:column}.stats-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.ops-list{grid-template-columns:1fr}.form-grid,.media-admin-grid{grid-template-columns:1fr}.header-actions{width:100%;flex-wrap:wrap}.header-actions input{flex:1}.crm-modal-grid{grid-template-columns:1fr;padding:18px}.crm-field-grid{grid-template-columns:1fr}.crm-modal-header{height:auto;padding:18px;align-items:flex-start}.crm-modal-title{font-size:18px}.point-actions{flex-direction:column}}
  </style>
</head>
<body>
  ...
  <main class="main-content"><header class="page-header"><div><div class="page-title" id="pageTitle">營運統計</div><div class="page-subtitle" id="pageSubtitle">以 HookTea 後台結構管理 CRM、商城、點數與經銷商歸屬</div></div><div class="header-actions"><span class="status-badge" id="systemStatus">連線中</span><input id="adminToken" type="password" placeholder="Admin token"><button class="btn-outline" id="saveToken">儲存</button><button class="btn-green-main" id="refreshAll">更新</button></div></header><div class="content">
    ...
    <section class="view" id="view-media"><div class="media-admin-grid"><section class="panel"><div class="panel-header"><div><div class="section-title" id="mediaFormTitle">新增影音</div><div class="muted">管理媒體牆的店家自有影音</div></div><button class="btn-outline btn-small" id="newMediaVideo">新增</button></div><div style="padding:16px"><input type="hidden" id="mediaVideoId"><div class="form-grid"><label class="full-span"><span class="input-label">標題</span><input id="mediaVideoTitle" maxlength="120" placeholder="例如：重口味現場直擊"></label><label><span class="input-label">標籤</span><input id="mediaVideoLabel" maxlength="60" value="官方影音"></label><label><span class="input-label">按鈕文字</span><input id="mediaVideoSourceLabel" maxlength="60" value="開啟店家影音"></label><label class="full-span"><span class="input-label">影音 HTTPS 網址</span><input id="mediaVideoUrl" class="mono" inputmode="url" placeholder="https://..."></label><label><span class="input-label">播放方式</span><select id="mediaVideoMode"><option value="external">外開原始影音</option><option value="html5">頁內 HTML5 播放</option></select></label><label><span class="input-label">排序</span><input id="mediaVideoOrder" type="number" value="0"></label><label><span class="input-label">狀態</span><select id="mediaVideoStatus"><option value="active">啟用</option><option value="inactive">停用</option></select></label><label class="full-span"><span class="input-label">封面 HTTPS 網址（選填）</span><input id="mediaVideoPoster" class="mono" inputmode="url" placeholder="https://..."></label><label class="full-span"><span class="input-label">內容說明</span><textarea id="mediaVideoDescription" class="media-description" maxlength="1000"></textarea></label></div><div class="media-help">LINE 簽名網址通常會過期；長期內容建議使用固定 HTTPS 網址。頁內播放只適用正確回傳 video/* 的來源。</div><div class="media-form-actions" style="margin-top:14px"><span class="muted" id="mediaVideoStatusText"></span><button class="btn-outline btn-small" id="cancelMediaVideo">清除</button><button class="btn-green-main btn-small" id="saveMediaVideo">儲存影音</button></div></div></section><section class="panel"><div class="panel-header"><div><div class="section-title">影音內容</div><div class="muted" id="mediaVideoCount">讀取中</div></div><a class="btn-outline btn-small" href="/media-wall" target="_blank" rel="noopener noreferrer">查看前台</a></div><div class="admin-table-container"><table class="admin-table"><thead><tr><th>排序</th><th>影音</th><th>播放</th><th>狀態</th><th>更新時間</th><th>操作</th></tr></thead><tbody id="mediaVideoRows"></tbody></table></div></section></div></section>
  ...
  <script>
    ...
    function setView(view){ qsa(".nav-item").forEach(btn => btn.classList.toggle("nav-active", btn.dataset.view === view)); qsa(".view").forEach(section => section.classList.toggle("active", section.id === "view-" + view)); const title = titles[view] || titles.dashboard; qs("#pageTitle").textContent = title[0]; qs("#pageSubtitle").textContent = title[1]; const saveButton=qs("#saveToken"); const refreshButton=qs("#refreshAll"); if(saveButton) saveButton.textContent=view==="settings"?"儲存設定":"儲存 Token"; if(refreshButton) refreshButton.textContent="重新載入"; }
    ...
    on("#saveToken", "click", async () => { const button=qs("#saveToken"); const settingsActive=qs("#view-settings")?.classList.contains("active"); const idleLabel=settingsActive?"儲存設定":"儲存 Token"; button.disabled=true; button.textContent="儲存中…"; adminToken=qs("#adminToken").value.trim(); localStorage.setItem("gusys_admin_token",adminToken); setLoginCover(false); try{ if(settingsActive){ await saveSettings("settingsStatus"); }else{ await loadAll(true); } }finally{ button.disabled=false; button.textContent=idleLabel; } }); on("#loginSubmit", "click", async () => { adminToken = qs("#loginToken").value.trim(); qs("#adminToken").value = adminToken; localStorage.setItem("gusys_admin_token", adminToken); setLoginCover(false); await loadAll(true); }); on("#refreshAll", "click", async () => { const button=qs("#refreshAll"); const settingsActive=qs("#view-settings")?.classList.contains("active"); button.disabled=true; button.textContent="更新中…"; try{ if(settingsActive){await loadSettings(true);}else{await loadAll(true);} }finally{ button.disabled=false; button.textContent="重新載入"; } }); on("#refreshAudit", "click", loadAudit);
    ...
    on("#newMediaVideo", "click", resetMediaVideoForm); on("#cancelMediaVideo", "click", resetMediaVideoForm); on("#saveMediaVideo", "click", saveMediaVideo); on("#mediaVideoRows", "click", event => { const edit=event.target.closest("[data-edit-media]"); const remove=event.target.closest("[data-delete-media]"); if(edit) editMediaVideo(edit.dataset.editMedia); if(remove) deleteMediaVideo(remove.dataset.deleteMedia); });
    ...
    function resetMediaVideoForm(){ qs("#mediaVideoId").value=""; qs("#mediaVideoTitle").value=""; qs("#mediaVideoDescription").value=""; qs("#mediaVideoLabel").value="官方影音"; qs("#mediaVideoUrl").value=""; qs("#mediaVideoSourceLabel").value="開啟店家影音"; qs("#mediaVideoMode").value="external"; qs("#mediaVideoPoster").value=""; qs("#mediaVideoOrder").value="0"; qs("#mediaVideoStatus").value="active"; qs("#mediaFormTitle").textContent="新增影音"; qs("#mediaVideoStatusText").textContent=""; }
    function mediaVideoPayload(){ return {id:qs("#mediaVideoId").value.trim(),title:qs("#mediaVideoTitle").value.trim(),description:qs("#mediaVideoDescription").value.trim(),mediaLabel:qs("#mediaVideoLabel").value.trim(),sourceUrl:qs("#mediaVideoUrl").value.trim(),sourceLabel:qs("#mediaVideoSourceLabel").value.trim(),playbackMode:qs("#mediaVideoMode").value,posterUrl:qs("#mediaVideoPoster").value.trim(),displayOrder:Number(qs("#mediaVideoOrder").value||0),status:qs("#mediaVideoStatus").value}; }
    function renderMediaVideos(){ qs("#mediaVideoCount").textContent="共 "+money(adminMediaVideos.length)+" 則"; qs("#mediaVideoRows").innerHTML=adminMediaVideos.map(item=>'<tr><td>'+money(item.displayOrder)+'</td><td><strong>'+esc(item.title)+'</strong><div class="muted">'+esc(item.mediaLabel||"官方影音")+'</div><div class="media-url mono" title="'+esc(item.sourceUrl)+'">'+esc(item.sourceUrl)+'</div></td><td>'+badge(item.playbackMode==="html5"?"頁內播放":"外開連結",item.playbackMode==="html5"?"":"warn")+'</td><td>'+badge(item.status==="active"?"啟用":"停用",item.status==="active"?"":"warn")+'</td><td class="mono">'+esc(item.updatedAt||item.createdAt||"")+'</td><td><div class="media-row-actions"><button class="btn-outline btn-small" data-edit-media="'+esc(item.id)+'">編輯</button><button class="btn-outline btn-small" data-delete-media="'+esc(item.id)+'">刪除</button></div></td></tr>').join("")||tableEmpty(6,"尚未建立影音內容"); }
    async function loadMediaVideos(){ adminMediaVideos=await api("/api/admin/media-videos"); renderMediaVideos(); }
    function editMediaVideo(id){ const item=adminMediaVideos.find(video=>video.id===id); if(!item)return; qs("#mediaVideoId").value=item.id||""; qs("#mediaVideoTitle").value=item.title||""; qs("#mediaVideoDescription").value=item.description||""; qs("#mediaVideoLabel").value=item.mediaLabel||"官方影音"; qs("#mediaVideoUrl").value=item.sourceUrl||""; qs("#mediaVideoSourceLabel").value=item.sourceLabel||"開啟店家影音"; qs("#mediaVideoMode").value=item.playbackMode||"external"; qs("#mediaVideoPoster").value=item.posterUrl||""; qs("#mediaVideoOrder").value=item.displayOrder||0; qs("#mediaVideoStatus").value=item.status||"active"; qs("#mediaFormTitle").textContent="編輯影音"; qs("#mediaVideoStatusText").textContent=""; window.scrollTo({top:0,behavior:"smooth"}); }
    async function saveMediaVideo(){ const payload=mediaVideoPayload(); if(!payload.title||!payload.sourceUrl){ qs("#mediaVideoStatusText").textContent="請填寫標題與影音網址"; return; } qs("#mediaVideoStatusText").textContent="儲存中"; try{ await api("/api/admin/media-videos",{method:"POST",body:JSON.stringify(payload)}); await loadMediaVideos(); resetMediaVideoForm(); qs("#mediaVideoStatusText").textContent="影音已儲存，前台已同步"; }catch(err){ qs("#mediaVideoStatusText").textContent="儲存失敗："+err.message; } }
    async function deleteMediaVideo(id){ const item=adminMediaVideos.find(video=>video.id===id); if(!item||!confirm("刪除影音「"+item.title+"」？"))return; try{ await api("/api/admin/media-videos/"+encodeURIComponent(id),{method:"DELETE"}); if(qs("#mediaVideoId").value===id)resetMediaVideoForm(); await loadMediaVideos(); qs("#mediaVideoStatusText").textContent="影音已刪除，前台已同步"; }catch(err){ qs("#mediaVideoStatusText").textContent="刪除失敗："+err.message; } }
