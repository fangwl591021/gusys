...
<body>
  <main class="app">
    <header class="topbar">
      ...
      <div class="top-actions"><button class="shield" type="button" aria-label="會員安全">盾</button><div class="avatar" id="lineAvatar">U</div></div>
    </header>
    ...
    <nav class="bottom-nav" aria-label="商城導覽">
      <button class="nav-item active" type="button"><span class="nav-ico">⌖</span><span>探索</span></button>
      <button class="nav-item" type="button"><span class="nav-ico">▣</span><span>預約</span></button>
      <button class="nav-item" type="button"><span class="nav-ico">▰</span><span>商城</span></button>
      <button class="nav-item" type="button"><span class="nav-ico">▶</span><span>影音</span></button>
      <button class="nav-item" type="button"><span class="nav-ico">▤</span><span>紀錄</span></button>
      <button class="nav-item" type="button"><span class="nav-ico">●</span><span>我的</span></button>
    </nav>
  </main>
  ...
  <script>
    ...
    async function initLiff(){try{if(!settings.liff_id||!window.liff)return;await liff.init({liffId:settings.liff_id});if(!liff.isLoggedIn())return;const profile=await liff.getProfile();$("lineUserId").value=profile.userId||"";if(!$("shippingName").value)$("shippingName").value=profile.displayName||"";if(profile.pictureUrl)$("lineAvatar").innerHTML='<img src="'+esc(profile.pictureUrl)+'" alt="">';}catch(e){console.warn(e)}}
    ...
    $("shippingCarrier").onchange=renderShipping;$("paymentMethod").onchange=renderPayment;$("submitOrder").onclick=submitOrder;$("cartFab").onclick=openCart;$("closeCart").onclick=closeCart;$("sheetMask").onclick=closeCart;renderAll();initLiff();
  </script>
</body>
