...
<body>
  <aside class="sidebar"><div class="sidebar-brand"><div class="brand-title">Gusys 管理站</div><div class="brand-subtitle">經銷商 LINE OA 後台</div></div><nav class="nav" id="nav"><div class="nav-group-header">營運中心</div><button class="nav-item nav-active" data-view="dashboard">營運統計</button><button class="nav-item" data-view="sales">業務 QR</button><button class="nav-item" data-view="customers">客戶 CRM</button><button class="nav-item" data-view="inventory">進銷存商品</button><button class="nav-item" data-view="reports">業績報表</button><div class="nav-group-header">營運工具</div><button class="nav-item" data-view="messages">LINE 訊息</button><button class="nav-item" data-view="ai">AI 後台監控</button><button class="nav-item" data-view="webhooks">雙 Webhook</button><button class="nav-item" data-view="settings">系統設定</button></nav></aside>
  <main class="main-content"><header class="page-header"><div><div class="page-title" id="pageTitle">營運統計</div><div class="page-subtitle" id="pageSubtitle">公司 > 業務 > 用戶的經銷商 LINE OA 控制台</div></div><div class="header-actions"><span class="status-badge" id="systemStatus">連線中</span><input id="adminToken" type="password" placeholder="Admin token"><button class="btn-outline" id="saveToken">儲存</button><button class="btn-green-main" id="refreshAll">更新</button></div></header><div class="content">
  ...
  <script>
    ...
    const titles = {dashboard:["營運統計","即時掌握業務、客戶、商品、LINE 訊息與母站轉送"],sales:["業務 QR","建立業務專屬 QR，作為日後業績歸屬依據"],customers:["客戶 CRM","查看用戶與業務的綁定關係"],inventory:["進銷存商品","管理商品、售價、成本與安全庫存"],reports:["業績報表","每月業務績效與毛利彙整"],messages:["LINE 訊息","查詢 LINE OA 對話紀錄"],ai:["AI 後台監控","追蹤高風險訊息、分類與建議動作"],webhooks:["雙 Webhook","查看母站轉送狀態，不顯示整段 HTML 原始碼"],settings:["系統設定","確認 Worker、LINE Webhook 與母站 Webhook"]};
    ...
    qs("#runAi").onclick = async () => { qs("#aiRunStatus").textContent = "分析中"; try{ await api("/api/ai-monitor/analyze",{method:"POST",body:JSON.stringify({limit:30})}); qs("#aiRunStatus").textContent = "完成"; await loadAi(); }catch(err){ qs("#aiRunStatus").textContent = err.message; } }; qs("#loadReport").onclick = () => loadReports();
