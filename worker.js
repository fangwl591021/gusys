...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--dark);font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;font-size:14px}button,input{font:inherit}
    ...
    @media(max-width:980px){.sidebar{position:static;width:auto}.main-content{margin-left:0}.page-header{position:static;align-items:flex-start;flex-direction:column}.stats-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.ops-list{grid-template-columns:1fr}.form-grid{grid-template-columns:1fr}.header-actions{width:100%;flex-wrap:wrap}.header-actions input{flex:1}}
  </style>
</head>
<body>
  ...
  <main class="main-content"><header class="page-header"><div><div class="page-title" id="pageTitle">營運統計</div><div class="page-subtitle" id="pageSubtitle">以 HookTea 後台結構管理 CRM、商城、點數與經銷商歸屬</div></div><div class="header-actions"><span class="status-badge" id="systemStatus">連線中</span><input id="adminToken" type="password" placeholder="Admin token"><button class="btn-outline" id="saveToken">儲存</button><button class="btn-green-main" id="refreshAll">更新</button></div></header><div class="content">
    ...
    <section class="view" id="view-customers"><section class="panel"><div class="panel-header"><div><div class="section-title">客戶 CRM</div><div class="muted">HookTea 模式：會員列表先看 LINE 名稱與 UID，再進 CRM 檔案</div></div><input id="customerSearch" placeholder="搜尋 LINE 名稱 / UID"></div><div class="admin-table-container"><table class="admin-table"><thead><tr><th>LINE 名稱</th><th>LINE UID</th><th>最近互動</th><th>操作</th></tr></thead><tbody id="customerRows"></tbody></table></div></section></section>
    <section class="view" id="view-member_detail"><section class="panel"><div class="panel-header"><div><div class="section-title">CRM 檔案</div><div class="muted" id="customerDetailSub">會員詳細資料、業務歸屬與點數操作</div></div><button class="btn-outline btn-small" id="backToCustomers">返回 CRM 列表</button></div><div class="panel-body"><div class="ops-list" id="customerDetailCards"></div></div></section><section class="panel"><div class="panel-header"><div><div class="section-title">點數管理</div><div class="muted">仿 HookTea：進 CRM 檔案後讀取點數，並可手動贈點 / 扣點</div></div><span class="status-badge warn" id="pointStatus">尚未讀取</span></div><div class="panel-body"><div class="form-grid"><input id="pointAmount" type="number" placeholder="異動點數"><input id="pointReason" placeholder="原因 / 備註"><button class="btn-green-main" id="grantPoints">贈點</button><button class="btn-outline" id="deductPoints">扣點</button></div></div><div class="admin-table-container"><table class="admin-table"><thead><tr><th>時間</th><th>類型</th><th>點數</th><th>內容</th><th>餘額</th></tr></thead><tbody id="pointRows"></tbody></table></div></section></section>
  ...
  <script>
    ...
    function setView(view){ qsa(".nav-item").forEach(btn => btn.classList.toggle("nav-active", btn.dataset.view === view || (view === "member_detail" && btn.dataset.view === "customers"))); qsa(".view").forEach(section => section.classList.toggle("active", section.id === "view-" + view)); const title = titles[view] || titles.dashboard; qs("#pageTitle").textContent = title[0]; qs("#pageSubtitle").textContent = title[1]; }
    ...
    async function loadCustomers(){ adminCustomers = await api("/api/admin/customers"); renderCustomers(); } function renderCustomers(){ const q = (qs("#customerSearch")?.value || "").trim().toLowerCase(); const rows = adminCustomers.filter(r => !q || [r.displayName,r.lineUserId,r.salesName,r.salesCode].join(" ").toLowerCase().includes(q)); qs("#customerRows").innerHTML = rows.map(r => '<tr><td><strong>'+esc(r.displayName||"LINE 會員")+'</strong><div class="muted">'+esc(r.status||"active")+'</div></td><td class="mono">'+esc(r.lineUserId)+'</td><td>'+esc(r.lastMessageAt||"尚無互動")+'<div class="muted">訊息 '+money(r.messageCount)+' 筆</div></td><td><button class="btn-outline btn-small" data-crm="'+esc(r.lineUserId)+'">CRM 檔案</button></td></tr>').join("") || tableEmpty(4,"尚無會員"); qsa("[data-crm]").forEach(btn => btn.onclick = () => openCustomerDetail(btn.dataset.crm)); } async function openCustomerDetail(lineUserId){ activeCustomer = adminCustomers.find(r => r.lineUserId === lineUserId); if(!activeCustomer) return; setView("member_detail"); qs("#customerDetailSub").textContent = (activeCustomer.displayName || "LINE 會員") + " / " + activeCustomer.lineUserId; qs("#customerDetailCards").innerHTML = [["LINE 名稱",activeCustomer.displayName||"LINE 會員"],["LINE UID",activeCustomer.lineUserId],["業務歸屬",(activeCustomer.salesName||"未綁定") + (activeCustomer.salesCode ? " / " + activeCustomer.salesCode : "")],["加入時間",activeCustomer.firstSeenAt||""],["最近互動",activeCustomer.lastMessageAt||"尚無互動"],["訊息數",money(activeCustomer.messageCount)+" 筆"]].map(i => '<div class="ops-item"><div class="ops-label">'+esc(i[0])+'</div><div class="ops-value">'+esc(i[1])+'</div></div>').join(""); await loadCustomerPoints(); } async function loadCustomerPoints(){ if(!activeCustomer) return; qs("#pointStatus").textContent = "點數讀取中"; try{ const result = await api("/api/points/list?lineUserId=" + encodeURIComponent(activeCustomer.lineUserId)); const logs = Array.isArray(result.logs) ? result.logs : (Array.isArray(result.data) ? result.data : (Array.isArray(result.items) ? result.items : [])); qs("#pointStatus").textContent = result.skipped ? (result.error || "點數 API 尚未設定") : "點數已更新"; qs("#pointRows").innerHTML = logs.map(log => '<tr><td>'+esc(log.created_at||log.createdAt||log.date||"")+'</td><td>'+esc(log.event_name||log.eventName||log.type||"")+'</td><td>'+esc(log.points||log.amount||log.point||"")+'</td><td>'+esc(log.event_content||log.eventContent||log.reason||"")+'</td><td>'+esc(log.balance||log.after_balance||"")+'</td></tr>').join("") || tableEmpty(5,"目前沒有點數流水資料"); }catch(err){ qs("#pointStatus").textContent = err.message; qs("#pointRows").innerHTML = tableEmpty(5,"點數資料讀取失敗"); } } async function submitPointAdjust(type){ if(!activeCustomer) return; const raw = Number(qs("#pointAmount").value || 0); const reason = qs("#pointReason").value.trim(); if(!raw || raw <= 0){ qs("#pointStatus").textContent = "請輸入大於 0 的點數"; return; } if(!reason){ qs("#pointStatus").textContent = "請填寫異動原因"; return; } const points = type === "spend" ? -Math.abs(raw) : Math.abs(raw); qs("#pointStatus").textContent = "送出中"; try{ const result = await api("/api/points/adjust",{method:"POST",body:JSON.stringify({lineUserId:activeCustomer.lineUserId,eventName:type === "spend" ? "後台扣點" : "後台贈點",eventContent:reason,points})}); qs("#pointStatus").textContent = result.skipped ? (result.error || "點數 API 尚未設定") : "點數調整完成"; await loadCustomerPoints(); }catch(err){ qs("#pointStatus").textContent = err.message; } }
