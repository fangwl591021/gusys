...
<body>
  ...
  <script>
    ...
    settingFields().forEach(el => el.addEventListener("input", () => { const key = el.dataset.setting; qsa("[data-setting=\""+key+"\"]").forEach(other => { if(other !== el) other.value = el.value; }); hookteaSettings = collectSettings(); renderSettingsPreview(); renderLiffLinks(); }));
