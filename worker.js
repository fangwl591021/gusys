...
async function sendPaidBroadcast(request, env) {
  ...
  requireDb(env);
  const payload = await request.json().catch(() => ({}));
  return json({ ok: true, data: await executePaidBroadcast(env, payload, { allowTest: true }) });
}
    ...
    <section class="view" id="view-audit"><section class="panel"><div class="panel-header"><div class="section-title">操作紀錄</div><span class="status-badge">Webhook 事件</span></div><div class="admin-table-container"><table class="admin-table"><thead><tr><th>類型</th><th>目前紀錄來源</th><th>狀態</th></tr></thead><tbody><tr><td>LINE 訊息</td><td>/api/admin/line-messages</td><td>已串接</td></tr><tr><td>母站轉送</td><td>/api/admin/webhooks</td><td>已串接</td></tr><tr><td>後台操作</td><td>audit_logs</td><td>待建立</td></tr></tbody></table></div></section></section>
      ...
      <div class="settings-wrap" style="max-width:1180px">
        <section class="settings-card">
          <div class="panel-header" style="padding:0 0 16px;border-bottom:0"><div><div class="section-title">HookTea 商城模組</div><div class="muted">集中管理 HookTea 前台商城模組，不再放在系統設定頁。</div></div><div><span class="muted" id="shopModuleStatus"></span> <button class="btn-green-main btn-small" id="saveShopModules">儲存模組設定</button></div></div>
          <div class="module-tabs"><button class="module-tab active" data-shop-tab="source">資料來源</button><button class="module-tab" data-shop-tab="home">首頁模組</button><button class="module-tab" data-shop-tab="member">會員專區</button><button class="module-tab" data-shop-tab="payment">付款訂單</button></div>
        </section>
        <div class="shop-layout">
          <div>
            <section class="settings-card shop-tab-panel" id="shopTab-source"><div class="section-title">商城資料來源</div><div class="form-grid" style="margin-top:18px"><label><span class="input-label">商城模式</span><select data-setting="shop_module"><option value="hooktea">HookTea 本地商品</option><option value="huaxu">外部商品 API</option></select></label><label><span class="input-label">前台 LIFF ID</span><input data-setting="liff_id" class="mono" placeholder="2007674851-lQljb6Cm"></label><label class="full-span"><span class="input-label">外部商品 API URL</span><input data-setting="huaxu_products_url" class="mono" placeholder="https://.../products.json 或外部商品 API"></label><label class="full-span"><span class="input-label">外部 API Key</span><input data-setting="huaxu_api_key" type="password" class="mono" placeholder="可留空；正式建議放 Cloudflare Worker Secret"></label></div></section>
            <section class="settings-card shop-tab-panel" id="shopTab-home" style="display:none"><div class="section-title">首頁主視覺與分類</div><div class="form-grid" style="margin-top:18px"><label><span class="input-label">主標題</span><input data-setting="shop_hero_title" placeholder="HookTea 精選 LINE 限定商城"></label><label><span class="input-label">標籤文字</span><input data-setting="shop_hero_badge" placeholder="新會員限定"></label><label class="full-span"><span class="input-label">副標文字</span><textarea data-setting="shop_hero_subtitle" placeholder="HookTea LINE 限定商城，訂單送出後會進入 HookTea 後台訂單維護。"></textarea></label><label class="full-span"><span class="input-label">分類列</span><input data-setting="shop_categories" placeholder="熱門商品,線上購物商品,虎克茶,新會員優惠,本月活動"></label></div></section>
            <section class="settings-card shop-tab-panel" id="shopTab-member" style="display:none"><div class="section-title">會員專區模組</div><div class="form-grid" style="margin-top:18px"><label><span class="input-label">會員卡標題</span><input data-setting="shop_member_title" placeholder="會員專區"></label><label><span class="input-label">每日簽到按鈕</span><input data-setting="shop_checkin_label" placeholder="每日簽到領點"></label><label class="full-span"><span class="input-label">會員功能</span><input data-setting="shop_member_modules" placeholder="點數記錄,分享好友,推薦成果,個人基本資料"></label></div><div class="settings-note">會員模組會保留 LINE profile、入口參數與推薦來源，訂單會寫入 entryUrl / entryParams。</div></section>
            <section class="settings-card shop-tab-panel" id="shopTab-payment" style="display:none"><div class="section-title">付款與訂單模組</div><div class="form-grid" style="margin-top:18px"><label><span class="input-label">付款方式</span><input data-setting="shop_payment_methods" placeholder="LINEPAY,REMITTANCE,COD"></label><label><span class="input-label">LINE Pay 環境</span><select data-setting="linepay_env"><option value="sandbox">Sandbox 測試</option><option value="production">Production 正式</option></select></label><label class="full-span"><span class="input-label">匯款資訊</span><textarea data-setting="remittance_info"></textarea></label></div></section>
          </div>
          <aside class="settings-card"><div class="section-title">模組預覽</div><div class="phone-preview" style="margin-top:14px"><div style="padding:12px 16px;border-bottom:1px solid rgba(255,255,255,.12);font-weight:900">HookTea 購物商城</div><div class="phone-hero"><span class="phone-badge" id="previewBadge">新會員限定</span><div class="phone-title" id="previewTitle">HookTea 精選 LINE 限定商城</div><div class="phone-subtitle" id="previewSubtitle">訂單送出後會進入 HookTea 後台訂單維護。</div></div><div style="padding:14px;display:grid;grid-template-columns:1fr 1fr;gap:12px"><div style="height:86px;border-radius:8px;background:rgba(255,255,255,.12)"></div><div style="height:86px;border-radius:8px;background:rgba(255,255,255,.12)"></div></div></div><div class="settings-note">這裡只管理商城模組設定；商品內容仍在「商城商品」頁維護。</div></aside>
        </div>
      </div>
    </section>
